import React, { useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import POS from "./POS";
import NouvelleVente from "./NouvelleVente";
import CommandesAttente from "./CommandesAttente";
import Historique from "./Historique";
import Retours from "./Retours";
import Packs from "./Packs";
import Clients from "./Clients";
import NewOrder from "../restaurants/commandes/NewOrder";

const sousModules = [
  { id: "POS", label: "POS" },
  { id: "Nouvelle", label: "Nouvelle Vente" },
  { id: "Attente", label: "Commandes en Attente" },
  { id: "Historique", label: "Historique" },
  { id: "Retours", label: "Retours & Échanges" },
  { id: "Packs", label: "Packs" },
  { id: "Clients", label: "Clients (CRM)" },
];

const Ventes = ({ darkMode }) => {
  const [activeTab, setActiveTab] = useState("POS");

  const renderContent = () => {
    switch (activeTab) {
      case "POS":
        return <NewOrder/>;
      case "Nouvelle":
        return <NouvelleVente />;
      case "Attente":
        return <CommandesAttente />;
      case "Historique":
        return <Historique />;
      case "Retours":
        return <Retours />;
      case "Packs":
        return <Packs />;
      case "Clients":
        return <Clients />;
      default:
        return <POS />;
    }
  };

  return (
    <div className={`transition-colors duration-500 ${darkMode ? "dark" : ""}`}>
      <h1 className="text-2xl font-bold mb-4 text-gray-800 dark:text-gray-100">
        Module Ventes
      </h1>

      {/* Navigation horizontale */}
      <div className="flex flex-wrap gap-2 bg-gray-50 dark:bg-gray-800 p-3 rounded-lg shadow mb-4">
        {sousModules.map((item) => (
          <button
            key={item.id}
            onClick={() => setActiveTab(item.id)}
            className={`px-4 py-2 rounded-lg font-medium transition ${
              activeTab === item.id
                ? "bg-green-500 text-white"
                : "bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 hover:bg-green-100 dark:hover:bg-green-600"
            }`}
          >
            {item.label}
          </button>
        ))}
      </div>

      {/* Contenu dynamique avec transition */}
      <div className="bg-white dark:bg-gray-900 p-6 rounded-lg shadow">
        <AnimatePresence mode="wait">
          <motion.div
            key={activeTab}
            initial={{ opacity: 0, x: 50 }}
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: -50 }}
            transition={{ duration: 0.4 }}
          >
            {renderContent()}
          </motion.div>
        </AnimatePresence>
      </div>
    </div>
  );
};

export default Ventes;
